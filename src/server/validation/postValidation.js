...
module.exports = {
  //매거진 생성 유효성
  validatePostCreateReq: (data) => {
    const postCreateReqSchema = Joi.object({
      user_id: Joi.string()
        .required()
    ...
    const { error, value } = postCreateReqSchema.validate(data);
  ...
  // 매거진 수정 유효성
  validatePostUpdateReq: (data) => {
    const postUpdateReqSchema = Joi.object({
      user_id: Joi.string()
        .required()
        .error(() => new Error("사용자 ID는 필수입니다.")),
      nickname: Joi.string()
    ...
    const { error, value } = postUpdateReqSchema.validate(data);
